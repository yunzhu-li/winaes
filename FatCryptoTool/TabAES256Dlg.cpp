...
void CTabAES256Dlg::OnBnClickedBtnStart()
{
	...
	if(!this->_bDecrypt)
	{
		CPassCDlg _passcdlg;
		_passcdlg.DoModal();
		if(!_tcslen(_passcdlg._passC)){this->_ctl_edit_pass.SetFocus(); return;}
		if(_tcscmp(_lpszKey, _passcdlg._passC))
		{
			this->MessageBox(_T("The Passwords do not match."), _T("Prompt"), MB_ICONEXCLAMATION);
			this->_ctl_edit_pass.SetFocus();
			return;
		}
	}
