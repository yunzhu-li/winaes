...
// FatCryptoToolDlg.cpp : implementation file
//

#include "stdafx.h"
#include "FatCryptoTool.h"
#include "FatCryptoToolDlg.h"
...
CFatCryptoToolDlg::CFatCryptoToolDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CFatCryptoToolDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CFatCryptoToolDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, _TAB_MAIN, _ctl_tab_main);
}

BEGIN_MESSAGE_MAP(CFatCryptoToolDlg, CDialog)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDOK, &CFatCryptoToolDlg::OnBnClickedOk)
	ON_NOTIFY(TCN_SELCHANGE, _TAB_MAIN, &CFatCryptoToolDlg::OnTcnSelchangeTabMain)
	ON_BN_CLICKED(IDCANCEL, &CFatCryptoToolDlg::OnBnClickedCancel)
	ON_WM_DROPFILES()
END_MESSAGE_MAP()


// CFatCryptoToolDlg message handlers

BOOL CFatCryptoToolDlg::OnInitDialog()
{
...
void CFatCryptoToolDlg::OnPaint()
{
	if (IsIconic())
	{
...
HCURSOR CFatCryptoToolDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CFatCryptoToolDlg::OnBnClickedOk()
{
}

void CFatCryptoToolDlg::OnTcnSelchangeTabMain(NMHDR *pNMHDR, LRESULT *pResult)
{
	//476x240
	int _curTab;
...
void CFatCryptoToolDlg::OnBnClickedCancel()
{
	OnCancel();
}

void CFatCryptoToolDlg::OnDropFiles(HDROP hDropInfo)
{
